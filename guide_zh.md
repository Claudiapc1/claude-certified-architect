...
### 场景 7：对话式 AI 架构模式
您设计多轮对话系统，涵盖上下文窗口管理、指令在多轮中的持久性、记忆策略、安全执行的工具设计，以及处理模糊或矛盾的用户输入。

### 场景 8：Agentic AI 工具 *（内容缺失 — 欢迎贡献！）*
考生报告在考试中遇到了这一场景，但本指南尚未涵盖相关内容。如果您在真实考试中遇到过此场景的题目，请在 [GitHub Issues](https://github.com/paullarionov/claude-certified-architect/issues) 中分享，以便我们添加完整内容。您的贡献将帮助所有备考的人。
...
`messages` 数组使用两种对话角色,外加一种指令性角色:
- `user` — 用户消息,包括工具结果(以 `user` 角色消息中的 `tool_result` 内容块形式发送,而不是作为单独的 `tool` 角色)
- `assistant` — 模型响应(发送历史时包含),包括工具使用请求(`tool_use` 内容块)
- `system` — 可以通过顶层的 `system` 字段设置(从第一轮开始生效),也可以在 `messages` 中以 `{"role": "system", ...}` 内联方式添加(从该点开始生效,受特定放置规则约束 — 见下文)

工具结果不是以 `"tool"` 角色的消息发送的。它们是以 `user` 角色的消息发送的,其内容包含一个 `tool_result` 内容块:
...
`system` 也可以直接作为角色出现在 `messages` 数组中,而不仅仅通过顶层的 `system` 参数。这是为了在不使顶层 `system` 字段的缓存前缀失效的情况下,在对话中途添加指令。它有特定的放置规则:
- 必须紧跟在一个 `user` 轮次(包括含有 `tool_result` 块的轮次)之后,或紧跟在以服务器端工具使用结尾的 `assistant` 轮次之后。
- 必须位于某个 `assistant` 轮次之前,或作为数组的最后一个元素。
- 不能位于 `tool_use` 块与其对应的 `tool_result` 之间 — 这样做会返回 400 错误。
- 较晚出现的 `system` 消息(包括对话中途插入的)对于后续轮次,优先于较早的消息以及顶层的 `system` 字段。
...
**为什么选A：** 个人技能优先于同名的项目技能，因此重复使用 `commit` 这个名称，会让团队的技能仅对这位开发者悄悄被遮蔽——每当团队改进 `/commit` 时，他都收不到更新，还得自己记住当前运行的是同名命令下的另一个技能。将个人版本命名为 `/my-commit` 可以完全避免这种冲突：开发者仍可继续使用团队维护的 `/commit`，同时拥有一个名称清晰、独立的个人工作流技能，不会把两者搞混，也不会错过团队的更新。
...
- C) 在工具描述中添加详细指令和少样本示例，要求 Agent 始终先用 `dry_run=true` 调用并等待用户确认后再调用。
...
**为什么选 A：** Claude 的 API 完全无状态——每个请求必须在 `messages` 数组中包含完整的对话历史。随着对话增长，每个请求携带更多令牌，这直接增加了处理延迟和成本。模型不在调用之间维护任何内部状态（D 为假）。
...
**情境：** 您的 AI 辅导工具有一个 2,800 个令牌的系统提示词，定义了教学方法和适配规则。12 轮后，助手开始忽略熟练程度级别。
