...
В массиве `messages` используются две роли для диалога и одна инструктивная роль:
- `user` -- сообщения от пользователя, включая результаты вызова инструментов (отправляются как content block `tool_result` внутри сообщения с ролью `user`, а не как отдельная роль `tool`)
- `assistant` -- ответы модели (включаются при передаче истории), включая запросы на использование инструментов (content blocks `tool_use`)
- `system` -- можно задать через верхнеуровневое поле `system` (применяется с первого хода) или внутри `messages` как `{"role": "system", ...}` (применяется начиная с этого места, с учётом правил размещения — см. ниже)

Результаты вызова инструментов не отправляются как сообщение с ролью `"tool"`. Они отправляются как сообщение с ролью `user`, содержимое которого включает content block `tool_result`:
...
`system` также может появляться как роль непосредственно в массиве `messages`, а не только через верхнеуровневый параметр `system`. Это предназначено для добавления инструкций в середине разговора без инвалидации закешированного префикса из верхнеуровневого поля `system`. Действуют определённые правила размещения:
- Должна идти сразу после хода `user` (в том числе содержащего блоки `tool_result`) или хода `assistant`, завершающегося использованием серверного инструмента.
- Должна предшествовать ходу `assistant` либо быть последним элементом массива.
- Не может находиться между блоком `tool_use` и его `tool_result` -- в этом случае возвращается ошибка 400.
- Более поздние сообщения `system` (в том числе добавленные в середине разговора) имеют приоритет над более ранними и над верхнеуровневым полем `system` для последующих ходов.
...
- A) Создать персональную версию в `~/.claude/skills/` с другим именем, например `/my-commit`. **[ВЕРНО]**
- B) Добавить условную логику на основе имени пользователя в frontmatter проектного скилла.
- C) Создать персональную версию по пути `~/.claude/skills/commit/SKILL.md` с тем же именем.
- D) Установить `override: true` в frontmatter персонального скилла для приоритета над проектной версией.

**Почему A:** Персональные скиллы имеют приоритет над проектными скиллами с тем же именем, поэтому повторное использование имени `commit` незаметно скроет скилл команды именно для этого разработчика — он перестанет получать обновления всякий раз, когда команда улучшает `/commit`, и должен будет помнить, что запускает другой скилл под тем же именем команды. Назвав персональный вариант `/my-commit`, этот конфликт полностью устраняется: разработчик продолжает использовать поддерживаемый командой `/commit` и получает отдельный, ясно названный скилл для личного рабочего процесса, без риска перепутать их или пропустить обновления команды.
