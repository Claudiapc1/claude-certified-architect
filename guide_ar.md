...
تستخدم مصفوفة `messages` دورين حواريين بالإضافة إلى دور توجيهي واحد:
- `user` — رسائل المستخدم، بما في ذلك نتائج الأدوات (تُرسل ككتلة محتوى `tool_result` ضمن رسالة بدور `user`، وليست دورًا منفصلاً باسم `tool`)
- `assistant` — ردود النموذج (تُدرج عند إرسال السجل)، بما في ذلك طلبات استخدام الأدوات (كتل محتوى `tool_use`)
- `system` — يمكن تعيينه عبر الحقل العلوي `system` (يسري من الدور الأول) أو ضمن `messages` كـ `{"role": "system", ...}` (يسري من تلك النقطة فصاعدًا، وفق قواعد وضع محددة — انظر أدناه)

نتائج الأدوات لا تُرسل كرسالة بدور `"tool"`. بل تُرسل كرسالة بدور `user` يتضمن محتواها كتلة محتوى `tool_result`:
...
يمكن لـ `system` أن يظهر أيضًا كدور مباشرة ضمن مصفوفة `messages`، وليس فقط عبر معامل `system` العلوي. الهدف من ذلك هو إضافة تعليمات في منتصف المحادثة دون إبطال البادئة المخزنة مؤقتًا (cached prefix) من الحقل العلوي `system`. ولهذا الأسلوب قواعد وضع محددة:
- يجب أن يأتي مباشرة بعد دور `user` (بما في ذلك دور يحتوي على كتل `tool_result`) أو دور `assistant` ينتهي باستخدام أداة من جهة الخادم (server tool use).
- يجب أن يسبق دور `assistant` أو أن يكون آخر عنصر في المصفوفة.
- لا يمكن أن يقع بين كتلة `tool_use` ونتيجتها `tool_result` — فعل ذلك يُرجع خطأ 400.
- رسائل `system` اللاحقة (بما في ذلك تلك التي تظهر في منتصف المحادثة) لها الأولوية على الرسائل السابقة وعلى الحقل العلوي `system` بالنسبة للأدوار التالية.
...
- A) إنشاء نسخة شخصية تحت `~/.claude/skills/` باسم مختلف، مثل `/my-commit`. **[CORRECT]**
- B) إضافة منطق شرطي حسب اسم المستخدم في frontmatter الخاص بمهارة المشروع.
- C) إنشاء نسخة شخصية في `~/.claude/skills/commit/SKILL.md` بالاسم نفسه.
- D) تعيين `override: true` في frontmatter الخاص بالمهارة الشخصية لجعلها أعلى أولوية من نسخة المشروع.

**لماذا A:** المهارات الشخصية لها أولوية على مهارات المشروع عند استخدام الاسم نفسه، لذا فإن إعادة استخدام الاسم `commit` سيؤدي إلى حجب مهارة الفريق بصمت لهذا المطور وحده — لن يتلقى التحديثات كلما حسّن الفريق مهارة `/commit`، وسيحتاج إلى تذكّر أنه يشغّل مهارة مختلفة تحت نفس الأمر. تسمية النسخة الشخصية `/my-commit` تتجنب هذا التعارض تمامًا: يستمر المطور في استخدام مهارة `/commit` التي يصونها الفريق، ويحصل على مهارة منفصلة وواضحة الاسم لسير عمله الشخصي، دون خطر الخلط بينهما أو فقدان تحديثات الفريق.
